...
> - [ ] files of non interest store such as: `[.dsstore, react packages, .gitignore`, store  in an array are exclude from memory save.
> - [ ] Data from files is read and specific keyword **@trace** follow by artifact identifiers `REQ` `VS` `ADR` are scan
